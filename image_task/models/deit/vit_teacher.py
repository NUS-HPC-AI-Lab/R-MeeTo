...
class Attention(nn.Module):
    ...
    def forward(self, x, size=None
        ...
        x = self.proj(x)
        x = self.proj_drop(x)
        return x, q.mean(dim=1), k.mean(dim=1), v.mean(dim=1)
...
class TeacherBlock(nn.Module):
    ...
    def forward(self, x):
        attn, q, k, v = self.drop_path(self.attn(self.norm1(x)))
        x = x + attn
...
@register_model
def RMeeTo_deit_small_teacher(pretrained=False, model_pth=None, **kwargs):
