...
class Block(nn.Module):
    ...
    def forward(
            self, hidden_states: Tensor, residual: Optional[Tensor] = None, inference_params=None, policy=None
    ):
...
class VisionMambaTeacher(nn.Module):
    def __init__(self,
        ...
        # Representation layer作用是将transformer的输出映射到一个新的空间，这个空间的维度是representation_size
    ...
    def forward_features(self, x, inference_params=None, if_random_cls_token_position=False,
        ...
        if self.if_cls_token:
            if self.use_middle_cls_token:
                tokens = torch.cat([hidden_states[:, :token_position, :], hidden_states[:, token_position:, :]], dim=1)
                return hidden_states[:, token_position, :], tokens
            elif if_random_cls_token_position:
                tokens = torch.cat([hidden_states[:, :token_position, :], hidden_states[:, token_position:, :]], dim=1)
                return hidden_states[:, token_position, :], tokens
            else:
    ...
    def forward(self, x, return_features=False, inference_params=None, if_random_cls_token_position=False,
                if_random_token_rank=False):
        cls, tokens = self.forward_features(x, inference_params,
...
@register_model
def RMeeTo_tiny_teacher(pretrained=False, model_pth=None, **kwargs):
...
@register_model
def RMeeTo_small_teacher(pretrained=False, model_pth=None, **kwargs):
    model = VisionMambaTeacher(
...
@register_model
def RMeeTo_base_teacher(pretrained=False, model_pth=None, **kwargs):
    model = VisionMambaTeacher(
