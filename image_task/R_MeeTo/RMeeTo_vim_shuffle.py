...
def shuffle(x1, x2, shuffle_rate, size):
    if shuffle_rate > 0:
        total_tokens = x1.shape[1]
        num_shuffle = int(total_tokens * shuffle_rate)

        shuffle_indices = torch.randperm(total_tokens)[:num_shuffle]
        non_shuffle_indices = torch.tensor([i for i in range(total_tokens) if i not in shuffle_indices])
        random_permutation = torch.randperm(shuffle_indices.shape[0])

        size[:, shuffle_indices] = size[:, shuffle_indices][:, random_permutation]
        x1[:, shuffle_indices] = x1[:, shuffle_indices][:, random_permutation]
        x2[:, shuffle_indices] = x2[:, shuffle_indices][:, random_permutation]

        size[:, non_shuffle_indices] = size[:, non_shuffle_indices]
        x1[:, non_shuffle_indices] = x1[:, non_shuffle_indices]
        x2[:, non_shuffle_indices] = x2[:, non_shuffle_indices]
...
class RMeeToBlock(Block):
    def __init__(
            ...
            class_token=False, if_pruning=False, if_order=True, if_visualize=False, if_shuffle=False,
            shuffle_rate: float = 0.0
    ):
        super().__init__(dim, mixer_cls, norm_cls, fused_add_norm, residual_in_fp32, drop_path)
        self.merge = RMeeTo_Merge(class_token, num_prune, if_pruning, if_order, distance, metric)
        ...
        if self.if_merge:  # metric
            ...
            if self.if_shuffle:
                ...
                if shuffle_rate > 0:
                    ...
                    cls_residual = residual[:, token_position, :]
                    cls_size = size[:, token_position, :]

                    hidden_states = torch.cat(
                        (hidden_states[:, :token_position, :], hidden_states[:, token_position + 1:, :]), dim=1)
                    residual = torch.cat((residual[:, :token_position, :], residual[:, token_position + 1:, :]), dim=1)
                    size = torch.cat((size[:, :token_position, :], size[:, token_position + 1:, :]), dim=1)

                    hidden_states, residual, size = shuffle(hidden_states, residual, shuffle_rate, size)
                    hidden_states = torch.cat((hidden_states[:, :token_position, :], cls_token.unsqueeze(1),
                                               hidden_states[:, token_position:, :]), dim=1)
                    residual = torch.cat(
                        (residual[:, :token_position, :], cls_residual.unsqueeze(1), residual[:, token_position:, :]),
                        dim=1)
                    size = torch.cat((size[:, :token_position, :], cls_size.unsqueeze(1), size[:, token_position:, :]),
                                     dim=1)
...
class R_MeeTo_Mamba_shuffle(VisionMamba):
    def __init__(self,
                 img_size=224,
                 patch_size=16,
                 stride=16,
        ...
        with torch.cuda.amp.autocast(enabled=False):
            for i, block in enumerate(self.layers):
                if i == 0:
                    ...
                    layer_flops += mamba_flops
                    flops += mamba_flops
        flops += patch_embedding_flops  # change to?
...
@register_model
def RMeeTo_tiny_shuffle(pretrained=False, model_pth=None, merge_para={}, **kwargs):
    if merge_para == {}:
        merge_para = {"num_prune": 5, "distance": "cosine", "metric": "X", "merge_interval": 2, "if_prune": False,
                      "if_order": True, "cls_token": True, "if_merge": True, "if_merge_odd": False, "if_shuffle": True,
                      "shuffle_rate": 0.0}
    num_prune, distance, metric, merge_interval, if_prune, if_order, class_token, if_merge, if_merge_odd, if_shuffle, shuffle_rate = \
    merge_para["num_prune"], merge_para["distance"], merge_para["metric"], merge_para["merge_interval"], merge_para[
        ...
        "if_merge_odd"], merge_para["if_shuffle"], merge_para["shuffle_rate"]
    print('shuffle_rate:', shuffle_rate)
    model = R_MeeTo_Mamba_shuffle(
        ...
        if_order=if_order, if_merge=if_merge, if_merge_odd=if_merge_odd, shuffle_rate=shuffle_rate,
        if_shuffle=if_shuffle, **kwargs)
...
@register_model
def RMeeTo_small_shuffle(pretrained=False, model_pth=None, merge_para={}, **kwargs):
    if merge_para == {}:
        merge_para = {"num_prune": 11, "distance": "cosine", "metric": "X", "merge_interval": 2, "if_prune": False,
                      "if_order": True, "cls_token": True, "if_merge": True, "if_merge_odd": False, "if_shuffle": True,
                      "shuffle_rate": 0.0}

    num_prune, distance, metric, merge_interval, if_prune, if_order, cls_token, if_merge, if_merge_odd, if_shuffle, shuffle_rate = \
    merge_para['num_prune'], merge_para['distance'], merge_para['metric'], merge_para['merge_interval'], merge_para[
        'if_prune'], merge_para['if_order'], merge_para['class_token'], merge_para['if_merge'], merge_para[
        'if_merge_odd'], merge_para['if_shuffle'], merge_para['shuffle_rate']
    print('shuffle_rate:', shuffle_rate)
...
@register_model
def RMeeTo_base_shuffle(pretrained=False, model_pth=None, merge_para={}, **kwargs):
    if merge_para == {}:
        merge_para = {"num_prune": 11, "distance": "cosine", "metric": "X", "merge_interval": 2, "if_prune": False,
                      "if_order": True, "cls_token": True, "if_merge": True, "if_merge_odd": False, "if_shuffle": True,
                      "shuffle_rate": 0.0}
