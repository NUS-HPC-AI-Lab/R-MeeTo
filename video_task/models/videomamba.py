...
from .R_MeeTo_merge import RMeeTo_Merge

MODEL_PATH = None
assert MODEL_PATH is not None, "Please set the MODEL_PATH variable to the path of the model weights. Example: MODEL_PATH = ./pretrained/video/"
_MODELS = {
    ...
    "videomamba_m16_in1k": os.path.join(MODEL_PATH, "videomamba_m16_k400_f8_res224.pth"),
}
...
class Block(nn.Module):
    ...
    def forward(
        ...
        if isinstance(hidden_states, tuple):
            hidden_states_return = hidden_states[0]
            B = hidden_states[1]
            C = hidden_states[2]
            delta = hidden_states[3]
            return hidden_states_return, residual, B, C, delta
        else:
...
@register_model  # tiny
def videomamba_R_MeeTo_tiny(pretrained=False, **kwargs):
    model = R_MeeTo_Mamba(
        patch_size=16,
        embed_dim=192,
        depth=24,
...
@register_model
def videomamba_R_MeeTo_small(pretrained=False, **kwargs):
    model = R_MeeTo_Mamba(
        patch_size=16,
        embed_dim=384,
        depth=24,
...
@register_model
def videomamba_R_MeeTo_middle(pretrained=False, **kwargs):
    model = R_MeeTo_Mamba(
        patch_size=16,
        embed_dim=576,
        depth=32,
