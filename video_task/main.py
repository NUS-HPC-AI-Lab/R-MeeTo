...
from utils.engine import train_one_epoch, validation_one_epoch, final_test, merge
...
def get_args():
    ...
    # merge
    parser.add_argument('--num_prune', default=88, type=int)
    parser.add_argument('--distance', default='cosine', type=str)
    parser.add_argument('--metric', default='X', type=str)
    parser.add_argument('--merge_interval', default=2, type=int)
    parser.add_argument('--if_order', action='store_true', help='if order')
    parser.add_argument('--if_prune', action='store_true', help='if prune')
...
def main(args, ds_init):
    ...
    # (sample,target,_) = next(iter(data_loader_val))
    ...
    if 'deit' in args.model:
        ...
        if 'R_MeeTo' in args.model:
            print("Use R_MeeTo model")
            print('num_prune = ', args.num_prune)
            model = create_model(
                args.model,
    ...
    if args.finetune:
        ...
        for key in all_keys:  # 这项操作的目的是？
            if key.startswith('backbone.'):
                new_dict[key[9:]] = checkpoint_model[key]  # 打印出模型进行检查
    ...
    if args.eval:
        print("test FLOPs")
        get_flops(model, img_size=224, show_detail=True)
